## Adsorption concerted electron transfer - full NRR reaction
## Li+ + S* + e- <-> Li
...
class CatalyticModel:
    def __init__(self,const_parameters,seioptions, atoler, rtoler, t_steps, x_steps):
        ...
        self.SCP2_d = pybamm.Parameter("Surface coverage of P2 [mol cm-2]")
        ...
        self.k2_d = pybamm.InputParameter("Forward Catalytic Rate 2 [mol-1 cm3 s-1]")
        self.k1b_d = pybamm.InputParameter("Backward Catalytic Rate 1 [s-1]")
        self.k2b_d = pybamm.InputParameter("Backward Catalytic Rate 2 [s-1]")
        ...
        self.scp2_nd = self.SCP2_d * self.G_0 # no units
    ...
    def model(self, sweep="forward", CS=None, CY=None, CA=None, CB=None, SCP=None, SCP2=None, SCX=None, SCZ=None,Ee=None):
        ...
        sc_p2 = pybamm.Variable("P2(ads) [non-dim]")
        ...
        BV1 = self.k0 * ((c_at_electrode_s * (sc_x) *  BV_red1 *pybamm.exp((-self.G_)*(sc_p-sc_p2))) 
                        - ((sc_p-sc_p2) * BV_ox1*pybamm.exp((self.G-self.G_)*(sc_p-sc_p2)))) 
        BV2 = self.k0 * ((c_at_electrode_s * (sc_p) *  BV_red2 *pybamm.exp((-self.G_)*sc_p2)) \
                        - (sc_p2 * BV_ox2*pybamm.exp((self.G-self.G_)*sc_p2)))
        
        #time derivatives
        dSdt =(pybamm.div(pybamm.grad(c_s)) * self.d_S)
        dYdt =(pybamm.div(pybamm.grad(c_y)) * self.d_Y)
        dAdt =(pybamm.div(pybamm.grad(c_a)) * self.d_A)
        dBdt =(pybamm.div(pybamm.grad(c_b)) * self.d_B)
        dsPdt = 
        dsP2dt = 
        dsXdt = 
        dsZdt = 
