## Main function
import catalyticmodel06_b as cm
import time
import re
...
def main():
    ...
    srate = [100]
    k0 = [1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3]
    Ru = 0
    Cdl = 0
    atol = 1e-9
    rtol = 1e-9
    ...
    for o in k0:   
        ...
        const_parameters = {
            ...
            "Temperature [K]": T,
            "Standard Unity Concentration [mol cm-3]": 1,
            ...
            "Electrode Radius [cm]": 0.01,
            "Voltage start [V]": 40,
            "Voltage reverse [V]": -40,
            "Voltage amplitude [V]": 0.0,
            "Scan Rate [V s-1]": srate[-1],
    ...
    for u in range(len(E_ds)):    
        plt.plot(E_ds[u], (I_ds[u]), label="v = " + str(k0[u]) + " mV/s")
