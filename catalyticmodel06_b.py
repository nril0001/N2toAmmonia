...
class CatalyticModel:
    def __init__(self,const_parameters,seioptions, atoler, rtoler, t_steps, x_steps, solver):
        ...
        self.E0 = self.E0_d  #no units
        self.k0 = self.k0_d  #no units
        self.Cdl = self.Cdl_d  #no units
        self.Ru = self.Ru_d  #no units
        
        #Diffusion coefficients
        self.d_S = self.DS_d  #no units
        self.d_P = self.DP_d  #no units
        self.d_max = pybamm.maximum(self.d_S, self.d_P) #no units
        
        #Concentrations
        self.c0 = self.C0 #no units
        self.cs_nd = self.CS_d  #no units
        self.cp_nd = self.CP_d  #no units
        
        #Surface concentrations
        self.G_max = self.Gamma #no units
        self.csa_nd = self.CSa_d  # no units
        self.cpa_nd = self.CPa_d  # no units

        self.E_start = self.E_start_d  #no units
        self.E_reverse = self.E_reverse_d  #no units
        
        #creating time scale and non-dimensionalizing
        self.Tmax_nd = (abs(self.E_start_d - self.E_reverse_d)/self.v) #no units
    ...
    def model(self, sweep="forward", Cs =None, Cpa = None, active=None ,Ee=None, Ea=None):
        ...
        BV = self.k0 * ((c_at_electrode_s * (1-c_p) *  BV_red) 
                        - (c_p * self.c0 * BV_ox)) 
