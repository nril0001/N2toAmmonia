...
def main():
    ...
    for i in files:
        ...
        cmodel = cm.CatalyticModel(const_parameters,seioptions, atol, rtol, steps)
        ...
        plt.legend()
        plt.grid()
        plt.savefig(output+"pybamm_S&P_cat05_dim_ko_"+str(i[5])+"_Ds_"+str(i[6])+"_Dp_"+str(i[7])+".png", dpi=600)
        np.savetxt(output+"CV_cat04_offset_k0_"+k+".dat", np.transpose(np.vstack((E_d, O_nd))))
        
        plt.cla()
        # plt.plot(E_d[:10000], (O_nd[:10000]), color = "red", label="PyBamm - S")
        # plt.plot(E_d[:10000], (R_nd[:10000]), color = "orange", label="PyBamm - P")
        plt.plot(voltage[:398], np.array(surfcon[:398])/surfcon[0], color = 'blue', linestyle = 'dashdot', label = 'Digielch - S')
        plt.plot(voltage[399:], np.array(surfcon[399:])/surfcon[0], color = 'green', linestyle = 'dashdot', label = 'Digielch - P')
        #plt.plot(E_d, cat, color = "orange", label="PyBamm - Cat")
        plt.title("Normalised Concentrations of S and P")
        ...
        plt.savefig(output+"digi_S&P_cat05_dim_ko_"+str(i[5])+"_Ds_"+str(i[6])+"_Dp_"+str(i[7])+".png", dpi=600)
